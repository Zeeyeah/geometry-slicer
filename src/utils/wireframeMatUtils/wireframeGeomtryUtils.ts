...
export function unindexBufferGeometry(bufferGeometry: THREE.BufferGeometry): void {
	...
	const newAttribData: Array<{
		...
		return {
			name: key,
			array: [],
			attribute: bufferGeometry.getAttribute(key) as THREE.BufferAttribute
		};
	});
	...
	newAttribData.forEach((data) => {
		const TypedArrayConstructor = data.attribute.array.constructor as new (
			data: number[]
		) => THREE.TypedArray;
		const newArray = new TypedArrayConstructor(data.array);
		const newAttribute = new THREE.BufferAttribute(newArray, data.attribute.itemSize);
